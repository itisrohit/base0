...
describe('Base0 Integration Tests', () => {
    ...
    test('8. List Documents with Filtering', async () => {
        const res = await apiFetch(`/projects/${projectId}/collections/${collectionId}/documents?filter[rating]=5`, {
            method: 'GET',
            headers: {
                Authorization: `Bearer ${apiKey}`,
            },
    ...
    test('14. Delete File', async () => {
        const res = await apiFetch(`/storage/buckets/${bucketId}/files/${fileId}?projectId=${projectId}`, {
            method: 'DELETE',
            headers: {
                Authorization: `Bearer ${accessToken}`,
            },
